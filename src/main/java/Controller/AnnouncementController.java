...
public class AnnouncementController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		if (action.equals("/List.an")) {
			...
			// 4. 어떻게 어디로 이동 할것인가?
			String path = "/JSP/Announcement/List.jsp";
			request.getRequestDispatcher(path).forward(request, response);
			
		} else if (action.equals("/Write.an")) {

			// 3. 어떻게 어디로 이동 할것인가?
			String path = "/JSP/Announcement/Write.jsp";
			...
			if (rs == 1) {
				...
				String path = "/JSP/Announcement/Write.jsp"; // 다시 작성 페이지로 돌아감
				request.getRequestDispatcher(path).forward(request, response);
			}
			...
			String path = "/JSP/Announcement/View.jsp";
			request.getRequestDispatcher(path).forward(request, response);
			...
			// 3. 어떻게 어디로 이동 할것인가?
			String path = "/JSP/Announcement/Edit.jsp";
			request.getRequestDispatcher(path).forward(request, response);
			...
			if (rs == 1) {
				...
				String path = "/JSP/Announcement/Edit.jsp"; // 다시 작성 페이지로 돌아감
				request.getRequestDispatcher(path).forward(request, response);
			}
